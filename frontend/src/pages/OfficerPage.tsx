...
export function OfficerPage(){
  ...
  useEffect(()=>{
    void refresh();
    void Promise.all([api.staffDepartments(),api.translationLanguages()])
      .then(([departmentList,languageList])=>{setDepartments(departmentList);setLanguages(languageList)})
  ...
  const shown=useMemo(()=>tickets.filter(item=>{
    ...
    const hay=`${t.ticket_code} ${t.category} ${t.location} ${t.department} ${item.citizen?.name||""} ${t.complaint}`.toLowerCase();
  ...
  return <Page title="Civic operations desk" eyebrow="Employee & administrator workspace" subtitle="Review the original citizen statement, translate it when needed, verify service context, route work safely and preserve an accountable history before taking action.">
    {notice&&<Toast notice={notice} onClose={()=>setNotice(null)}/>} 
    {selected&&<TicketDrawer key={selected.ticket.ticket_code} item={selected} departments={departments} languages={languages} onClose={()=>setSelected(null)} onChanged={async()=>{await refresh();setSelected(await api.platformTicket(selected.ticket.ticket_code))}} notify={setNotice}/>} 
    ...
    <Card className="mt-6 overflow-hidden border-slate-200 shadow-panel dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-3 border-b border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative max-w-xl flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/><input className="focus-field pl-9" placeholder="Search ticket, citizen, complaint, category or location…" value={search} onChange={e=>setSearch(e.target.value)}/></div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1240px] text-left text-sm">
          ...
          <tbody>{shown.map(item=>{const t=item.ticket;return <tr className="border-t border-slate-100 bg-white transition hover:bg-slate-50/70 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800/60" key={t.ticket_code}>
            ...
            <td className="px-4 py-4"><div className="font-semibold">{t.category}</div><div className="mt-1 max-w-56 line-clamp-2 text-xs leading-5 text-slate-500 dark:text-slate-400">{t.complaint}</div></td>
            <td className="px-4 py-4"><div className="max-w-48 line-clamp-2">{t.location}</div>{item.meta.ward&&<div className="mt-1 text-xs text-slate-400">{item.meta.ward}</div>}</td>
...
function TicketDrawer({item,departments,languages,onClose,onChanged,notify}:{item:EnrichedTicket;departments:string[];languages:TranslationLanguage[];onClose:()=>void;onChanged:()=>Promise<void>;notify:(n:NoticeState)=>void}){
  const t=item.ticket;
  const[officer,setOfficer]=useState(t.assigned_officer||"");
  ...
  async function upload(){if(!file)return;await act(()=>api.uploadEvidence(t.ticket_code,file,"RESOLUTION_EVIDENCE"),"Resolution evidence attached to the ticket.");setFile(null)}
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/55 backdrop-blur-sm">
    <div className="absolute inset-y-0 right-0 w-full max-w-3xl overflow-y-auto border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-[#0b1726]">
      <div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-200 bg-white/95 p-5 backdrop-blur dark:border-slate-800 dark:bg-[#0b1726]/95">
        <div><p className="text-xs font-black uppercase tracking-[.18em] text-teal-700 dark:text-teal-300">Staff ticket detail</p><h2 className="mt-1 text-2xl font-black text-slate-950 dark:text-white">{t.ticket_code}</h2></div>
      ...
      <div className="space-y-5 p-5">
        <section className="rounded-3xl bg-[linear-gradient(135deg,#0b1f3a,#0f766e)] p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.18em] text-teal-100">{t.category} · {t.language}</div><h3 className="mt-2 text-2xl font-black">Original citizen statement</h3><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-100">{t.complaint}</p></div><Badge value={t.priority}/></div>
        </section>
        ...
        <Card className="overflow-hidden border-teal-200 bg-gradient-to-br from-teal-50 to-cyan-50 shadow-sm dark:border-teal-900 dark:from-teal-950/30 dark:to-slate-900">
          <CardHeader><CardTitle className="flex items-center gap-2"><Languages className="text-teal-700 dark:text-teal-300" size={20}/>Translate citizen statement</CardTitle></CardHeader>
          <CardContent>
            <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">Choose the language you prefer for staff review. Only the complaint text is sent for translation; the citizen's original statement remains unchanged and authoritative.</p>
            ...
            {translation&&<div className="mt-4 rounded-2xl border border-teal-200 bg-white p-4 dark:border-teal-800 dark:bg-slate-950/40"><div className="flex flex-wrap items-center justify-between gap-2"><b className="text-sm text-slate-900 dark:text-white">{translation.target_language_name} translation</b><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Machine translation</span></div><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-700 dark:text-slate-200">{translation.translated_text}</p><p className="mt-3 text-xs text-slate-400">Verify names, addresses and safety-critical details against the original statement before taking action.</p></div>}
          </CardContent>
        </Card>
        ...
        <Card className="dark:border-slate-800 dark:bg-slate-900">
          <CardHeader><CardTitle>Ownership & routing</CardTitle></CardHeader>
          <CardContent className="space-y-5">
            <div><p className="mb-2 text-xs font-black uppercase tracking-wide text-slate-400">Officer / team assignment</p><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><input className="focus-field" value={officer} onChange={e=>setOfficer(e.target.value)} placeholder="Officer/team name"/><Button onClick={()=>void act(()=>api.assignTicket(t.ticket_code,officer),officer?`Assigned to ${officer}.`:"Ticket unassigned.")} disabled={busy}>Assign</Button></div></div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
              ...
              <div className="grid gap-3">
                ...
                <textarea className="focus-field min-h-20" value={reason} onChange={e=>setReason(e.target.value)} placeholder="Why should this ticket move to the selected department?"/>
                <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">For accountability, every transfer records the old department, new department, reason, staff identity and timestamp. If a different officer is already assigned, that assignment is cleared and active work is returned to the new department queue.</p>
        ...
        <Card className="dark:border-slate-800 dark:bg-slate-900"><CardHeader><CardTitle>Resolution evidence</CardTitle></CardHeader><CardContent><div className="space-y-2">{item.attachments.map(a=><a href={api.evidenceUrl(a.id)} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl border border-slate-200 p-3 text-sm dark:border-slate-700" key={a.id}><span><FileUp className="mr-2 inline text-teal-700" size={15}/>{a.original_name}</span><span className="text-xs text-slate-400">{a.kind.replaceAll("_"," ")}</span></a>)}</div><div className="mt-4 flex gap-2"><input type="file" className="min-w-0 flex-1 text-xs" accept="image/jpeg,image/png,image/webp,video/mp4,application/pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/><Button size="sm" variant="outline" onClick={()=>void upload()} disabled={!file||busy}>Upload</Button></div><textarea className="focus-field mt-4 min-h-28" value={note} onChange={e=>setNote(e.target.value)} placeholder="Describe what was done, what was inspected, and any follow-up required…"/><Button className="mt-3 w-full" onClick={()=>void act(()=>api.resolveWithNote(t.ticket_code,note),"Ticket marked resolved and sent for citizen confirmation.")} disabled={busy||note.trim().length<3}>Mark resolved with note</Button></CardContent></Card>

        <Card className="dark:border-slate-800 dark:bg-slate-900"><CardHeader><CardTitle>Activity history</CardTitle></CardHeader><CardContent className="space-y-4">{t.audit_events.map((e,i)=>{const transferEvent=e.event==="DEPARTMENT_TRANSFERRED";return <div className={transferEvent?"rounded-2xl border border-cyan-200 bg-cyan-50 p-4 dark:border-cyan-900 dark:bg-cyan-950/20":"border-l-2 border-teal-200 pl-4 dark:border-teal-800"} key={`${e.event}-${i}`}><b className="flex items-center gap-2 text-sm text-slate-900 dark:text-white">{transferEvent&&<ArrowRightLeft size={15} className="text-cyan-700 dark:text-cyan-300"/>}{e.event.replaceAll("_"," ")}</b><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{e.detail}</p><small className="text-slate-400">{new Date(e.created_at).toLocaleString()}</small></div>})}</CardContent></Card>
      </div>
    </div>
  </div>
}

function Info({icon,title,children}:{icon:ReactNode;title:string;children:ReactNode}){return <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[.16em] text-teal-700 dark:text-teal-300"><span className="[&>svg]:h-4 [&>svg]:w-4">{icon}</span>{title}</div><div className="space-y-1 text-sm text-slate-600 dark:text-slate-300">{children}</div></div>}
