...
from app.services.ticketing import IdempotencyConflict, InvalidStatusTransition, create_ticket, find_duplicates, simulate_breach, update_status
...
def water_payload(location: str = "Shivaji Nagar") -> TicketCreateRequest:
    return TicketCreateRequest(complaint="There has been no water supply in our area for three days and nobody is responding.", location=location)


def test_ticket_lifecycle_and_escalation():
    db = session(); payload = water_payload(); analysis = analyze_complaint(payload.complaint, supplied_location=payload.location)
    ticket = create_ticket(db, payload, analysis)
    assert ticket.ticket_code.startswith("CR-") and ticket.status == "SUBMITTED" and ticket.sla_state == "SAFE"
    escalated = simulate_breach(db, ticket)
    assert escalated.status == "ESCALATED" and escalated.sla_state == "BREACHED"
...
def test_duplicate_detection_across_same_category():
    db = session()
    first = TicketCreateRequest(complaint="Garbage has not been collected on FC Road for five days and it smells.", location="FC Road")
    ...
    matches = find_duplicates(db, analyze_complaint(second_text, supplied_location="FC Road"), second_text)
    assert matches and matches[0].ticket_code == created.ticket_code
...
def test_idempotency_key_reuse_with_different_payload_fails():
    db = session(); first_payload = water_payload(); first_analysis = analyze_complaint(first_payload.complaint, supplied_location=first_payload.location)
    create_ticket(db, first_payload, first_analysis, idempotency_key="citizen-submit-002")
    second_payload = water_payload("Kothrud"); second_analysis = analyze_complaint(second_payload.complaint, supplied_location=second_payload.location)
    try:
...
def test_resolved_ticket_cannot_be_reopened_by_status_patch():
    db = session(); payload = water_payload(); analysis = analyze_complaint(payload.complaint, supplied_location=payload.location)
    resolved = update_status(db, create_ticket(db, payload, analysis), "RESOLVED", note="Repair verified.")
    try:
